...
#Con training test al 70/30
#No	59	29.5		
#Si	141	70.5	

#Con training test al 50/50
#No	55	27.5		
#Si	145	72.5	
```
...
```{r}
trControl <- trainControl(method = "cv", number = 10)
data.partition <- createDataPartition(df.train$Gravedad, times = 1)
training <- df.train[data.partition$Resample1,]
testing <- df.train[-data.partition$Resample1,]
...
model3.1 <- train(x = select(training, -c(Gravedad, N_Leves, N_Muertos, N_Graves)), 
                y = training$Gravedad, 
                method = "AdaBag", 
                trControl = trControl,
                tuneGrid = expand.grid(mfinal = c(100,300,500), maxdepth = c(5,10,15))
                )

confusionMatrix(predict(model3.1, testing), testing$Gravedad, positive="Si") #Sale un accuracy del 0.9396

# Generando valores para test de kaggle del modelo que sea
predictions_list <- predict(model3.1, df.test)
...
#Con tunegrid
model4.1 <- train(x = select(training, -c(Gravedad, N_Leves, N_Muertos, N_Graves)), 
                y = training$Gravedad, 
                method = "AdaBag", 
                trControl = trControl,
                tuneGrid = expand.grid(mfinal = c(100,300,500), maxdepth = c(5,10,15))
...
confusionMatrix(predict(model4.1, testing), testing$Gravedad, positive="Si")

# Generando valores para test de kaggle del modelo que sea
predictions_list <- predict(model4.1, df.test)
...
model5 <- train(x = select(training, -c(Gravedad, N_Leves, N_Muertos, N_Graves)), 
                y = training$Gravedad, 
                method = "AdaBag", 
                trControl = trControl
                )

confusionMatrix(predict(model5, testing), testing$Gravedad, positive="Si")

# Generando valores para test de kaggle del modelo que sea
predictions_list <- predict(model5, df.test)
...
#Con tunegrid
model5.1 <- train(x = select(training, -c(Gravedad, N_Leves, N_Muertos, N_Graves)), 
                y = training$Gravedad, 
                method = "AdaBag", 
                trControl = trControl,
                tuneGrid = expand.grid(mfinal = c(100,300,500), maxdepth = c(5,10,15))
...
confusionMatrix(predict(model5.1, testing), testing$Gravedad, positive="Si")

# Generando valores para test de kaggle del modelo que sea
predictions_list <- predict(model5.1, df.test)
...
# Intentamos hacer poling con los modelos elegidos
...
#Para NO
model4.1P <- train(x = select(training, -c(Gravedad, N_Leves, N_Muertos, N_Graves)), 
                y = training$Gravedad, 
                method = "AdaBag", 
                trControl = trControl,
                tuneGrid = expand.grid(mfinal = c(500), maxdepth = c(15))
                )

predictions_list4.1P <- predict(model4.1P, df.test, type="prob")

#Para SI
model4P <- train(x = select(training, -c(Gravedad, N_Leves, N_Muertos, N_Graves)), 
                y = training$Gravedad, 
                method = "AdaBag", 
                trControl = trControl
                )

predictions_list4P <- predict(model4P, df.test, type="prob")
...
model1P <- train(x = select(training, -c(Gravedad, N_Leves, N_Muertos, N_Graves)), 
                y = training$Gravedad, 
                method = "ranger", 
                trControl = trControl, 
                num.trees = 500,
...
predictions_list1P <- predict(model1P, df.test, type="prob")

#Haciendo el test de kaggle
all_probs <- cbind(predictions_list4.1P, predictions_list4P, predictions_list1P)


results <- data.frame(1:200, 1)
names(results) <- c('Id', 'Prediction')

for (i in 1:200) {
  no_no <- all_probs[i,1]
  no_yes <- all_probs[i,2]
  yes_no <- all_probs[i,3]
  yes_yes <- all_probs[i,4]
  norm_no <- all_probs[i,5]
  norm_yes <- all_probs[i,6]
  
  # Con votos: sale equilibrado pero hay más no
  # votes <- c(0,0,0)
  # 
  # if (no_yes > 0.4) {
  #   votes[1] <- 1
  ...
  # }
  # 
  # results[i,2] <- round(mean(votes))
  
  #Con media: sale equilibrado
  #mean_yes <- mean(c(no_yes, yes_yes, norm_yes))
  #mean_no <- mean(c(no_no, yes_no, norm_no))
  # if (mean_yes > 0.5) {
  #   results[i, 2] <- 1
  # } else {
  #   results[i, 2] <- 0
  ...
  # Jugando con las probabilidades es muy a favor de los sí
  if (( no_yes > 0.4 & yes_yes > 0.75) | (no_yes > 0.4 & yes_yes > 0.75 & norm_yes > 0.75)) {
    results[i, 2] <- 1
  } else {
    if (no_no > 0.6 & norm_no > 0.65) {
      results[i, 2] <- 0
    } else { #Por defecto ponemos un 1
      print('-----YES-----')
      print(no_yes)
      print(yes_yes)
      print(norm_yes)
...
#Con testing con balanceo personalizado
data.partition <- createDataPartition(df.trainCustom$Gravedad, times = 1, p = 0.7)
training <- df.train[data.partition$Resample1,]
testing <- df.train[-data.partition$Resample1,]

results_testing <- data.frame(1:4787, 1)
names(results_testing) <- c('Id', 'Prediction')
all_probs_testing <- cbind(predict(model4.1P, testing, type="prob"), predict(model4P, testing, type="prob"), predict(model1P, testing, type="prob"))

for (i in 1:4787) {
  no_no <- all_probs_testing[i,1]
  no_yes <- all_probs_testing[i,2]
  yes_no <- all_probs_testing[i,3]
  yes_yes <- all_probs_testing[i,4]
  norm_no <- all_probs_testing[i,5]
  norm_yes <- all_probs_testing[i,6]
  
  if ((no_yes > 0.75 & yes_yes > 0.75) | (no_yes > 0.6 & yes_yes > 0.8 & norm_yes > 0.75)) {
    results_testing[i, 2] <- 1
  } else {
    if (no_no > 0.6 & norm_no > 0.65) {
      results_testing[i, 2] <- 0
    } else { #Por defecto ponemos un 1
      print('-----YES-----')
      print(no_yes)
      print(yes_yes)
...
confusionMatrix(   ifelse(results_testing[,2] == 1, 'Si', 'No'), testing$Grave, positive="Si")
