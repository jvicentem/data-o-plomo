...
model <- train(Gravedad ~ ., data=training, trControl=train_control, method="parRF", importance=T, preProcess=c('scale', 'center'), ntree=500, na.action = na.omit)
...
```{r}
df.result <- data.frame(1:200, predict(model, df.test))
...
```{r}
df.result.ada <- read.csv('/home/mountain/URJC/3-Datatons/17abril-URJC/data-o-plomo_LOCAL/results/2017_04_07_15_23_results_adaboost.csv', header = TRUE)

df.result.ada %>% group_by(Prediction) %>%
    summarise(count = n()) %>%
    mutate(percent = count/sum(count)*100)
