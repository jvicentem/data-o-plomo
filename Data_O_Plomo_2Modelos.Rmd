...
training_neg <- SMOTE(form = Gravedad ~ ., data = training, perc.over = 200, perc.under = 50)
...
model_parRF_neg <- train(Gravedad ~ ., data=training_neg, trControl=train_control, method="parRF", importance=T, preProcess=c('scale', 'center'), ntree=500, na.action = na.omit)
model_parRF_neg

model_parRF_pos <- train(Gravedad ~ ., data=training_pos, trControl=train_control, method="parRF", importance=T, preProcess=c('scale', 'center'), ntree=500, na.action = na.omit)
model_parRF_pos
...
test_model(model_LogitBoost_neg)
test_model(model_LogitBoost_pos)

predict(model_LogitBoost_neg, testing, type = "prob")


med <- (predict(model_LogitBoost_neg, df.test, type = "prob")[2] + predict(model_LogitBoost_pos, df.test, type = "prob")[2])/2

table(ifelse(med >= 0.5,1,0))

table(ifelse(predict(model_LogitBoost_pos, df.test, type = "prob")[2] >= 0.5,1,0))


predict(model_parRF_neg, df.test, type = "prob")[2]
...
length(which(med == 0.5))

df.result <- data.frame(Id = 1:200, Prediction = ifelse(med >= 0.5,1,0))


df.result <- read.csv(file = '/home/mountain/URJC/3-Datatons/17abril-URJC/data-o-plomo_LOCAL/results.csv')
