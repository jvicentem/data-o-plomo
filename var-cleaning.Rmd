...
#####################################################################################################################
  ...
  df$Tipo_Via <- as.factor(ifelse(df$Tipo_Via == "Via convencional sin carril lento", "Conv rapida", ifelse(df$Tipo_Via == "Autovia", "Autovia", "Resto")))
  ...
  df$Arcen <- as.factor(ifelse(df$Arcen == "Inexistente" | df$Arcen == "menor_1.5", "Estrecho", "Ancho"))
  
  ###################################################################################################################
  
  tipo_dia <- as.factor(case_when(
                df$Tipo_Dia %>% is_in(c(1, 4)) ~ 'NoGrave',
                df$Tipo_Dia %>% is_in(c(2, 3)) ~ 'Grave'
  ))

  table(tipo_dia)
  
  tipo_dia <- relevel(tipo_dia, ref = "NoGrave")
  df$Tipo_Dia <- tipo_dia
  ...
  df$Superficie <- as.factor(ifelse(df$Superficie == "Seca y limpia" | df$Superficie == "Aceite" | df$Superficie == "Otro", "TipoA", ifelse(df$Superficie == "Umbria" | df$Superficie == "Gravilla suelta" | df$Superficie == "Nevada", "TipoB", "TipoC")))
  
  ###################################################################################################################
  
  df$Interseccion_Acondicionamiento <- as.factor(ifelse(df$Interseccion_Acondicionamiento == "Carril central de espera" | df$Interseccion_Acondicionamiento == "Senal de giro izquierda", "TipoA", "TipoB"))
  
  ###################################################################################################################
  
  prioridad_regulara_por <- as.factor(case_when(
                                    df$Prioridad_Regulada_por %>% is_in(c(1, 2, 3, 6)) ~ 'Grave',
                                    df$Prioridad_Regulada_por %>% is_in(c(4, 5, 7, 8)) ~ 'NoGrave'
  ))
  
# agente + paso peatones + semáforo + stop -> Grave
# 
# ninguna + otra señal + ceda + marcas viales -> NoGrave
